...
// Logger:
var debug = logger( 'unary-loops:script' );
...
// Output directories:
var INCLUDE_DIR = path.resolve( __dirname, '..', 'include', 'stdlib', 'strided', 'base', 'unary' );
...
var MACROS = {
	'default': 'STDLIB_STRIDED_UNARY_LOOP_CLBK',
	'nocast': 'STDLIB_STRIDED_UNARY_LOOP_CLBK_RET_NONSCALAR',
	'cast': 'STDLIB_STRIDED_UNARY_LOOP_CLBK_ARG_CAST',
	'acast': 'STDLIB_STRIDED_UNARY_LOOP_CLBK_ARG_CAST_FCN',
	'rcast': 'STDLIB_STRIDED_UNARY_LOOP_CLBK_RET_CAST_FCN'
};
	...
	for ( i = 0; i < N; i++ ) {
		...
		for ( j = 0; j < casts.length; j++ ) {
			...
			if ( t2 === t1 ) {
				...
				if ( ch1 === 'f' ) {
					out.push( ch1+'_'+ch1+'_as_d_d' ); // e.g., `b_f` => `b_f_as_d_d`
				} else if ( ch1 === 'c' ) {
					out.push( ch1+'_'+ch1+'_as_z_z' ); // e.g., `b_c` => `b_c_as_z_z`
				}
				continue;
			}
			ch2 = dtypeChar( t2 );
	...
	if ( match2 ) {
		...
		if ( isComplexChar( ch1 ) ) {
			...
			if ( ch1 === ch2 ) {
				...
				macro = MACROS.nocast; // e.g., `c_z_as_c_z`
			}
		} else if ( ch1 === ch2 ) {
			macro = MACROS.cast;
			args.push( ct1 );
